...
public class MyTest : MonoBehaviour {
	...
	void Update () {
		...
		Vector3 f0 = (transform.position + (r * Vector3.forward) * distonce);
		...
		Vector3 f1 = (transform.position + (r0 * Vector3.forward) * distonce);
		Vector3 f2 = (transform.position + (r1 * Vector3.forward) * distonce);
	...
	{
		return Mathf.Abs((v0x * v1y + v1x * v2x + v2x * v0y - v1x * v0y - v2x * v1y - v0x * v2y) / 2f);
	}
	bool isINTriangle(Vector3 point,Vector3 v0,Vector3 v1,Vector3 v2)
	{
		...
		float t = triangleArea (v0x, v0y, v1x, v1y, x, y);
